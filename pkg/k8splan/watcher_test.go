...
func TestUpdateSecretConflictMergeSkipsKeysAbsentFromOurCopy(t *testing.T) {
	...
	// The reason PlanProgressKey was added to the merge list: the checkpoint must survive a retry.
	if string(retried.Data[PlanProgressKey]) != string(progress) {
		t.Errorf("expected merge key %q to carry our checkpoint %q, got %q", PlanProgressKey, progress, retried.Data[PlanProgressKey])
	}
}
