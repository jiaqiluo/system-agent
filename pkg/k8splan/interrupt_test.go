...
func TestParseInterruptAnnotation(t *testing.T) {
	...
	tests := []struct {
		...
		{name: `the value "true" requests the interrupt`, annotations: map[string]string{PlanPausedAnnotation: "true"}, want: true},
		{name: `the value "false" does not request the interrupt`, annotations: map[string]string{PlanPausedAnnotation: "false"}, want: false},
		{name: "a capitalised spelling is a configuration error", annotations: map[string]string{PlanPausedAnnotation: "True"}, wantErr: true},
		{name: "a present but empty value is a configuration error", annotations: map[string]string{PlanPausedAnnotation: ""}, wantErr: true},
		{name: "a trailing space is a configuration error", annotations: map[string]string{PlanPausedAnnotation: "true "}, wantErr: true},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := parseInterruptAnnotation(tt.annotations, PlanPausedAnnotation)
...
func TestStartInterruptWatchObservesAnnotations(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			steps:     []map[string]string{{}, {PlanPausedAnnotation: "true"}},
			wantPause: true,
		},
		{
...
func TestStartInterruptWatchIgnoresInvalidValuesUntilCorrected(t *testing.T) {
	...
	seq := &annotationSequence{steps: []map[string]string{{PlanPausedAnnotation: "yes"}}}
	...
	seq.push(map[string]string{PlanPausedAnnotation: "true"})
	waitForClose(t, "pause", pauseCh)
	if isClosed(cancelCh) {
...
func TestWriteInterruptOutcomeRetriesOnConflictAndPreservesTheCheckpoint(t *testing.T) {
	...
	if written.Annotations[PlanPausedAnnotation] != "true" {
		t.Error("the updates did not land on the freshly fetched object: the operator's annotation is missing")
	}
	if got := planapi.PlanState(written.Data[planapi.PlanStateKey]); got != PlanStatePaused {
		t.Errorf("wrote plan-state %q, want %q", got, PlanStatePaused)
	}
...
func TestWriteInterruptOutcomeSkipsTheUpdate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			serverData: map[string][]byte{planapi.PlanStateKey: []byte(PlanStatePaused)},
			updates:    map[string][]byte{planapi.PlanStateKey: []byte(PlanStatePaused)},
		},
		{
