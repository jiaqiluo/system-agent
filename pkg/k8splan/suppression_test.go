	...
	for _, row := range suppressionMatrix() {
		t.Run(row.name, func(t *testing.T) {
			...
			if row.wantUpdates == 0 {
				...
				if !bytes.Equal(result.Data[PlanProgressKey], secret.Data[PlanProgressKey]) {
					t.Errorf("expected the checkpoint to be honored verbatim, got %q want %q",
						result.Data[PlanProgressKey], secret.Data[PlanProgressKey])
				}
...
func TestChecksumFlowIgnoresAnInvalidAnnotationValue(t *testing.T) {
	...
	for _, value := range invalidAnnotationValues {
		t.Run(fmt.Sprintf("value=%q", value), func(t *testing.T) {
			...
			secret := newInterruptTestSecret(f.planBytes, map[string]string{PlanPausedAnnotation: value}, nil)
			rec := newInterruptRecorder(secret)
			sc := newInterruptTestController(t, rec)
			...
			for i, data := range []map[string][]byte{result.Data, settled.Data} {
				...
				if value, ok := data[PlanProgressKey]; ok {
					t.Errorf("reconcile %d materialised %q on a checksum-flow Secret, got %q", i, PlanProgressKey, value)
				}
			}

			want := "ignoring unsupported annotation in checksum flow key=" + PlanPausedAnnotation + " value=" + value
...
func TestRestartThenUnpauseResumesAtTheCheckpoint(t *testing.T) {
	...
	releases := []struct {
		...
		{name: "the operator set the annotation to false", annotations: map[string]string{PlanPausedAnnotation: "false"}},
	}
...
func TestCorrectedAnnotationValueSelfHeals(t *testing.T) {
	...
	// Pass 1: the typo.
	rec.setAnnotations(map[string]string{PlanPausedAnnotation: "yes"})
	mistyped := rec.get()
	...
	rec.setAnnotations(map[string]string{PlanPausedAnnotation: "true"})
	...
	rec.setAnnotations(map[string]string{PlanPausedAnnotation: "false"})
	if _, err = w.reconcileSecret(context.Background(), sc, rec.get(), 30*time.Second); err != nil {
...
func TestResumeThenPauseAgainRecordsTheNewerCheckpoint(t *testing.T) {
	...
	serveInterruptOnceApplyStarted(t, sc, PlanPausedAnnotation, second, func(served int) {
		if served > 1 {
...
func TestInterruptPathReEntryIsANoOp(t *testing.T) {
	...
	f := newSuppressionFixture(t)
	secret := newInterruptTestSecret(f.planBytes, map[string]string{PlanPausedAnnotation: "true"}, map[string][]byte{
		planapi.PlanStateKey: []byte(planapi.PlanStateInProgress),
...
func TestInterruptedApplyOutcomeSurvivesAConflictingWrite(t *testing.T) {
	...
	server := newInterruptTestSecret(planBytes, map[string]string{PlanPausedAnnotation: "true"}, map[string][]byte{
		planapi.PlanStateKey: []byte(planapi.PlanStateInProgress),
	})
	...
	if writes[0].Annotations[PlanPausedAnnotation] != "true" {
...
func TestInterruptOutcomeAbandonedWhenANewerPlanLanded(t *testing.T) {
	...
	server := newInterruptTestSecret(newPlanBytes, map[string]string{PlanPausedAnnotation: "true"}, map[string][]byte{
