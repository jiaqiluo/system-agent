...
// PlanCheckpointKey is the Secret data key holding the resume checkpoint (JSON, see PlanCheckpoint).
PlanCheckpointKey = "plan-progress"
// PlanStatePaused is a non-terminal plan-state: execution is held at an instruction boundary and
// will resume into PlanCheckpoint.ResumeState when the pause annotation is removed.
...
`PlanCheckpointKey` is appended to `secretConflictMergeKeys` so that the *clear* performed on a
...
// parsePlanProgress decodes the checkpoint stored under PlanCheckpointKey.
...
func parsePlanProgress(data map[string][]byte, checksum string) PlanCheckpoint
func marshalPlanProgress(p PlanCheckpoint) []byte
```
...
`secret_outcome.go`: `buildSecretDataUpdates` clears `PlanCheckpointKey` on both the success and
...
// PlanCheckpointKey is the Secret data key holding the agent's resume checkpoint.
PlanCheckpointKey = "plan-progress"
```
...
describes, and `parsePlanCheckpoint` returns the zero value on mismatch, so a stale record from a
